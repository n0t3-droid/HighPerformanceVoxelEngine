...
void main() {
    ...
    if (blockType == kWaterBlock) {
        ...
        lit = mix(deepColor, surfaceColor, 0.55);
        lit *= (0.62 + diffuse * 0.38);
        lit = mix(lit, vec3(0.55, 0.72, 0.90), fresnel * 0.45);

        // Surface (top face) is clearer than sides.
        float isTop = step(0.95, N.y);
        alpha = mix(0.82, 0.58, isTop);
        alpha = mix(alpha, 0.40, fresnel * 0.35);
    }
